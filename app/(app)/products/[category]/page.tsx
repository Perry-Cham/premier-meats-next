...
export default function Product_Page() {
  ...
  const [filtered, setFiltered] = useState<Record<string, any> | null>(null);
  const [totalProducts, setTotalProducts] = useState(0);
  const params = useParams<{ category: string }>();


  useEffect(() => {
    // FETCH PAGE CONTENT AND PRODUCTS FROM PAYLOADS API
    async function fetchData(category: string): Promise<void> {
      const product_res = await axios.get(
        `/api/products?where[category][equals]=${category}`,
      );
      const content_res = await axios.get(
        `/api/productpage?where[category][equals]=${category}`,
      );
      const products = product_res.data.docs;

      //Group them by subcategory
      ...
      const content = content_res.data.docs[0];

      //And finally set the total number of sub categories on the page.
      for (const [key, value] of Object.entries(processedProducts)){
         console.log(processedProducts[key])
         let p = 0
         p += processedProducts[key].length;
         setTotalProducts(p)
      } 

      console.log(processedProducts, products, content);
      setContent({ products: processedProducts, content: content });
